# Combo is a machine learning and classification algorithm in the
# imbalanced multiclass setting. It is a boosting algorithm that
# greedily minimizes an upper bound of the confusion matrix.
# Copyright (C) 2013 Sokol Koco, Aix-Marseille University
...
class ComboLearner(orange.Learner):
	...
	def __init__(self, learner = Orange.classification.tree.TreeLearner(max_depth=2, measure="gini", m_pruning=2), maxT = 10, verbose = True, alphaMethod=1):
	...
	def updateCosts(self, alpha, h):
		...
		for i in self._sample :
			...
			for k in range(self._numclass):
				if iClass != k :
					self._cost[iIndex][k] = (1./self._perclassnum[iClass])*math.exp(self._f[iIndex][k]-self._f[iIndex][iClass])
